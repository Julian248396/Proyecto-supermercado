...
def mostrar_inventario(inventario_tienda):
    """Muestra todos los productos con su información básica."""
...
def mostrar_detalle_producto(inventario_tienda):
    """
    Pide al usuario la clave de un producto y muestra toda
    su información incluyendo el diccionario anidado del proveedor,
    la tupla de unidad de medida, el set de etiquetas y el historial.
    """
...
def modulo_analisis(inventario_tienda):
    ...
    while True:
        ...
        match opcion:
            case "1":
                categorias = {p["categoria"] for p in inventario_tienda.values()}
                print(f"\n  Categorías únicas ({len(categorias)}): {categorias}")

            case "2":
                agotados = [p["nombre"] for p in inventario_tienda.values() if p["cantidad"] == 0]
                if agotados:
                    print("\n  Productos agotados:")
...
def modulo_info(inventario_tienda):
    ...
    while True:
        ...
        match opcion:
            ...
            case "2":
                for i, dato in enumerate(info_tienda):
                    print(f"  {i} - {dato}")
                try:
                    idx = int(input("  Ingrese el índice: "))
                    print(f"  Dato seleccionado: {info_tienda[idx]}")
...
def crud_productos(inventario_tienda):
    """
    CRUD completo de productos. Permite agregar, consultar,
    modificar campos y eliminar productos del inventario.
    """
    while True:
        mostrar_menu("GESTIÓN DE PRODUCTOS (CRUD)", [
            "Agregar producto",
            "Consultar producto",
...
def calcular_promedio_precios(historial):
    """
    Auxiliar 2: Recibe una lista de precios y retorna el promedio.
